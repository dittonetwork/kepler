...
func (s *KeeperTestSuite) TestEpochInfoBeginBlockChanges() {
	...
	for name, test := range tests {
		s.Run(name, func() {
			s.SetupTest()
			s.Ctx = s.Ctx.WithHeaderInfo(header.Info{Height: 1, Time: block1Time})
			initialEpoch := initializeBlankEpochInfoFields(test.initialEpochInfo, defaultIdentifier, defaultDuration)
			err := s.EpochsKeeper.AddEpochInfo(s.Ctx, initialEpoch)
			s.Require().NoError(err)
			err = s.EpochsKeeper.BeginBlocker(s.Ctx)
			s.Require().NoError(err)
			...
			for _, h := range heights {
				...
				s.Ctx = s.Ctx.WithHeaderInfo(header.Info{Height: int64(h), Time: test.blockHeightTimePairs[h]})
				err := s.EpochsKeeper.BeginBlocker(s.Ctx)
				s.Require().NoError(err)
			}
			expEpoch := initializeBlankEpochInfoFields(test.expEpochInfo, initialEpoch.Identifier, initialEpoch.Duration)
			actEpoch, err := s.EpochsKeeper.EpochInfo.Get(s.Ctx, initialEpoch.Identifier)
			s.Require().NoError(err)
			s.Require().Equal(expEpoch, actEpoch)
		})
	}
}
