...
func (hook *dummyEpochHook) AfterEpochEnd(_ context.Context, _ string, _ int64) error {
	if hook.shouldError {
		return dummyErr
	}
...
func (hook *dummyEpochHook) BeforeEpochStart(_ context.Context, _ string, _ int64) error {
	if hook.shouldError {
		return dummyErr
	}
	hook.successCounter += 1
